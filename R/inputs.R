...
#' @param msg Message type to dispatch.
#' @param value An Alpine.js expression for the displayed value (e.g.
#'   `"model.name"`). Keeps the input in sync with the model.
...
#'
#' @export
mvu_text_input <- function(label, msg, value = NULL, placeholder = NULL,
  ...
  if (!is.null(value)) attrs[["x-bind:value"]] <- value
...
#'
#' @export
mvu_password_input <- function(label, msg, value = NULL, placeholder = NULL,
                               debounce = NULL, class = "form-control", ...) {
  input_id <- random_id()
  attrs <- c(
...
#'
#' @examples
#' mvu_textarea_input("Bio", msg = "set_bio", value = "model.bio", rows = 4)
#'
#' @export
mvu_textarea_input <- function(label, msg, value = NULL, rows = 3,
...
#' @param msg Message type to dispatch.
#' @param selected An Alpine.js expression for the currently selected value
#'   (e.g. `"model.color"`). Keeps the dropdown in sync with the model.
...
mvu_select_input <- function(label, choices, msg, selected = NULL,
  ...
  if (!is.null(selected)) attrs[["x-bind:value"]] <- selected
  tagList(
    tags$label(class = "form-label", `for` = input_id, label),
...
#' @param label Label text.
#' @param msg Message type to dispatch.
#' @param value An Alpine.js expression for the checked state (e.g.
#'   `"model.agree"`). Keeps the checkbox in sync with the model.
...
#'
#' @export
mvu_checkbox_input <- function(label, msg, value = NULL,
                               class = "form-check", ...) {
  input_id <- random_id()
  attrs <- c(
...
#' @param selected An Alpine.js expression for the array of currently
#'   selected values (e.g. `"model.toppings"`). Keeps the checkboxes in
#'   sync with the model.
...
#' @param msg Message type to dispatch.
#' @param selected An Alpine.js expression for the currently selected value
#'   (e.g. `"model.pet"`). Keeps the radio buttons in sync with the model.
...
#' @param msg Message type to dispatch.
#' @param value An Alpine.js expression for the current value (e.g.
#'   `"model.volume"`). Keeps the slider in sync with the model.
...
#' @param label Label text.
#' @param msg Message type to dispatch.
#' @param value An Alpine.js expression for the current date value (e.g.
#'   `"model.birthday"`). Keeps the input in sync with the model.
...
#'
#' @examples
#' mvu_date_input("Birthday", msg = "set_birthday", value = "model.birthday")
#'
#' @export
mvu_date_input <- function(label, msg, value = NULL, min = NULL, max = NULL,
                           class = "form-control", ...) {
  input_id <- random_id()
  attrs <- c(
...
#' @param start,end Alpine.js expressions for the start and end date
#'   values. Keeps the inputs in sync with the model.
...
mvu_date_range_input <- function(label, msg_start, msg_end,
  ...
  if (!is.null(start)) start_attrs[["x-bind:value"]] <- start
  if (!is.null(end)) end_attrs[["x-bind:value"]] <- end
  if (!is.null(min)) {
